...
<div class="container">
  ...
  <div class="row">
    ...
    <div class="col s12">
      <div class="card">
        <div class="card-content">
          ...
          <br>
          <p><b>Etat de la commande: </b><span class="badge new left-align {{ $order->state->color }}"
              data-badge-caption="{{ $order->state->name }}"></span></p>
          @if($order->state->slug === 'carte' || $order->state->slug === 'erreur')
          @include('command.partials.stripe')
          @endif
          @if($order->invoice_id)
          <br>
          <td><a href="{{ route('invoice', $order->id) }}" class="waves-effect waves-light btn-small">Télécharger la
              facture</a></td>
          @endif
        </div>
      </div>
    ...
    <div class="col s12">
      <div class="card">
        <div class="card-content">
          <span class="card-title">Adresse de facturation @if($order->adresses->count() === 1)@endif</span>
  ...
  <div class="row">
    <a class="waves-effect waves-light btn" href="{{ route('commandes.index') }}"> <i
        class="material-icons left">chevron_left</i>Retour à mes commandes</a>
  </div>
</div>
@endsection

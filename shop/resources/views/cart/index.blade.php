          ...
          <div id="wrapper">
            ...
            <div class="row mb10px nowrape w-100">
              <form action="{{ route('panier.update', $item->id) }}" method="POST">
                ...
                <div class="col s3" style="line-height: 1.5">
                  <strong>{{ number_format($item->quantity * $item->price, 2, ',', ' ') }} €</strong></div>
              ...
              <form action="{{ route('panier.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <div class="col s1" style="line-height: 1.5"><i class="material-icons deleteItem"
                    style="cursor: pointer">delete</i></div>
              </form>
            </div>
            @endforeach
            <hr class="hr105"><br class="lh075">
